...
export async function POST(req: NextRequest) {
  try {
    ...
    if (WOMPI_EVENTS_SECRET) {
      const isValid = await verifyWompiSignature(
        [data?.transaction?.id, sent_at, event],
        signature?.checksum,
        WOMPI_EVENTS_SECRET
      )
    ...
    if (event === 'transaction.updated') {
      ...
      await supabase.from('wompi_transactions').update({
        wompi_id: txn.id,
        status: txn.status,
      ...
      if (txn.status === 'APPROVED') {
        const { data: localTxn } = await supabase
          ...
          .select('user_id, plan, amount_in_cents, currency')
          .eq('reference', txn.reference)
          .single()
        ...
        if (localTxn) {
          ...
          await supabase.from('subscriptions').upsert({
            ...
            amount_paid: localTxn.amount_in_cents / 100,
