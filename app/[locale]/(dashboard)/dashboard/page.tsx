...
async function getDashboardData(userId: string) {
  try {
    ...
    const revenue = ordersRes.data?.reduce((s, o) => s + (o.total ?? 0), 0) ?? 0
    const automationsFired = autoRes.data?.reduce((s, a) => s + (a.run_count ?? 0), 0) ?? 0
...
export default async function DashboardPage({ params }: PageProps) {
  const { locale } = params; const loc = locale as 'en' | 'es'
  let user = null; try { user = await getUser() } catch {}
  let profile = null; try { if (user) profile = await getUserProfile(user.id) } catch {}
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="font-display font-bold text-3xl mb-1" style={{ color: 'var(--text)' }}>{greeting}, {userName} ð</h1>
        <p className="text-sm" style={{ color: 'var(--text-3)' }}>{loc === 'es' ? 'AquÃ­ tienes el resumen de tu negocio.' : "Here's your business overview."}</p>
      </div>
      {!dbReady && (<div className="rounded-2xl p-5 flex items-start gap-4" style={{ background: 'rgba(237,25,102,0.08)', border: '1px solid rgba(237,25,102,0.25)' }}><AlertCircle size={20} style={{ color: 'var(--pink)', flexShrink: 0, marginTop: 2 }} /><div><p className="font-semibold text-sm mb-1" style={{ color: 'var(--text)' }}>{loc === 'es' ? 'Base de datos no configurada' : 'Database not set up yet'}</p><p className="text-sm" style={{ color: 'var(--text-3)' }}>{loc === 'es' ? 'Ve a Supabase â SQL Editor y ejecuta el archivo schema.sql.' : 'Go to Supabase â SQL Editor and run the schema.sql file to enable all features.'}</p></div></div>)}
      <div className="flex flex-wrap gap-3">
        {[{ label: loc === 'es' ? 'Nueva AutomatizaciÃ³n' : 'New Automation', href: `/${locale}/automations`, icon: 'â¡' },{ label: loc === 'es' ? 'Agregar Lead' : 'Add Lead', href: `/${locale}/leads/new`, icon: 'ð¤' },{label: loc === 'es' ? 'Ver AnalÃ­tica' : 'View Analytics', href: `/${locale}/analytics`, icon: 'ð ' }].map(({ label, href, icon }) => (
          <Link key={href} href={href} className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-all hover:opacity-90" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)', color: 'var(--text)' }}><span>{icon}</span>{label}</Link>
        ))}
      </div>
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[{ label: loc === 'es' ? 'Total Leads' : 'Total Leads', value: totalLeads, icon: 'ð¥', color: 'var(--pink)' },{ label: loc === 'es' ? 'Conversaciones' : 'Conversations', value: recentConvos.length, icon: 'ð¬', color: '#4a90d9' },{ label: loc === 'es' ? 'Automatizaciones' : 'Automations Fired', value: automationsFired, icon: 'â¡', color: '#22c55e' },{ label: loc === 'es' ? 'Ingresos' : 'Revenue', value: `$${revenue.toLocaleString()}`, icon: 'ð°', color: '#f59e0b' }].map(({ label, value, icon, color }) => (
          <div key={label} className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
            <div className="flex items-center justify-between mb-3"><span className="text-2xl">{icon}</span><div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${color}20` }}><div className="w-2 h-2 rounded-full" style={{ background: color }} /></div></div>
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(237,25,102,0.08), rgba(33,82,164,0.08))', border: '1px solid rgba(237,25,102,0.15)' }}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div><div className="flex items-center gap-2 mb-1"><span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /><span className="text-xs font-bold tracking-wider uppercase" style={{ color: 'var(--pink)' }}>{loc === 'es' ? 'Sistema activo' : 'System Active'}</span></div><h3 className="font-display font-bold text-lg mb-1" style={{ color: 'var(--text)' }}>{loc === 'es' ? 'JUT estÃ¡ listo para automatizar' : 'JUT is ready to automate'}</h3><p className="text-sm" style={{ color: 'var(--text-3)' }}>{loc === 'es' ? 'Conecta Instagram y crea tu primera automatizaciÃ³n.' : 'Connect Instagram and create your first automation.'}</p></div>
