...
import { Bot, Save, RefreshCw, Check, MessageSquare, Target, BookOpen, Tag, Play, Sparkles, Building2, User, Upload, Trash2, FileType, Globe, ChevronDown, ChevronUp } from 'lucide-react'
...
const TONES = [
  ...
  { id:'enthusiastic', label:'Enthusiastic', desc:'Energetic' },
  { id:'empathetic', label:'Empathetic', desc:'Understanding' },
]
...
export default function AgentPage() {
  ...
  const inp: React.CSSProperties = {
    ...
    color:'var(--text)', fontSize:14, outline:'none', marginTop:5,
  }

  const cols2: React.CSSProperties = { display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:14 }
  const cols3: React.CSSProperties = { display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'repeat(3,1fr)', gap:8 }
  const langGrid: React.CSSProperties = { display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'repeat(4,1fr)', gap:8 }
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, gap:12, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:'var(--text-3)', marginTop:2 }}>{agent.role} · {curTone.label} · {curLang.label}</div>
          </div>
        </div>
      ...
      {isMobile && (
        <div style={{ marginBottom:12, position:'relative' }}>
          <button onClick={() => setTabOpen(!tabOpen)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 14px', borderRadius:12, background:'var(--surface)', border:'1px solid var(--border-2)', cursor:'pointer', color:'var(--text)' }}>
            ...
            {tabOpen ? <ChevronUp size={16} color="var(--text-3)"/> : <ChevronDown size={16} color="var(--text-3)"/>}
          </button>
          {tabOpen && (
            <div style={{ position:'absolute', left:0, right:0, top:'100%', marginTop:4, background:'var(--surface)', border:'1px solid var(--border-2)', borderRadius:12, padding:6, zIndex:50, boxShadow:'0 8px 30px rgba(0,0,0,0.5)' }}>
              {TABS.map(t => (
            ...
            {tab === 'identity' && (
              <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
                ...
                <div>
                  ...
                  <div style={langGrid}>
                    {LANGUAGES.map(lang => (
                      <button key={lang.code} onClick={() => upd('language', lang.code)} style={{ display:'flex', alignItems:'center', gap:9, padding:'11px 12px', borderRadius:11, border:'2px solid '+(agent.language===lang.code?'var(--pink)':'var(--border-2)'), background:agent.language===lang.code?'rgba(237,25,102,0.07)':'var(--surface-2)', cursor:'pointer', transition:'all 0.15s' }}>
                ...
                <div style={{ padding:16, borderRadius:13, background:'var(--surface-2)', border:'1px solid var(--border-2)' }}>
                  ...
                  <div style={cols2}>
                    ...
                    <div><label style={{ fontSize:11, fontWeight:600, color:'var(--text-3)', display:'block' }}>Type</label><select value={agent.business_type} onChange={e=>upd('business_type',e.target.value)} style={{ ...inp, cursor:'pointer' }}><option value="">Select...</option>{BUSINESS_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
                  ...
                  <div style={cols3}>
                    {RESPONSE_LENGTHS.map(r => (
                      <button key={r.id} onClick={() => upd('response_length', r.id)} style={{ padding:'12px 10px', borderRadius:12, border:'2px solid '+(agent.response_length===r.id?'var(--pink)':'var(--border-2)'), background:agent.response_length===r.id?'rgba(237,25,102,0.07)':'var(--surface-2)', cursor:'pointer', textAlign:'center', transition:'all 0.15s' }}>
                        ...
                        <div style={{ fontSize:10, color:'var(--text-4)' }}>{r.desc}</div>
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
                    {['helpful','professional','empathetic','direct','creative','patient','proactive','concise','witty'].map(trait => {
                      ...
                      return (
                        <button key={trait} onClick={() => { const t=agent.personality_traits||[]; upd('personality_traits', sel?t.filter((x:string)=>x!==trait):[...t,trait]) }} style={{ padding:'6px 12px', borderRadius:999, fontSize:12, fontWeight:sel?600:400, border:'1px solid '+(sel?'var(--pink)':'var(--border-2)'), background:sel?'rgba(237,25,102,0.1)':'var(--surface-2)', color:sel?'var(--pink)':'var(--text-3)', cursor:'pointer', transition:'all 0.15s' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:12 }}>
                    {DOC_TYPES.map(dt => (
                      <button key={dt.type} onClick={() => { if(fileUploadRef.current){ fileUploadRef.current.dataset.type=dt.type; fileUploadRef.current.click() } }} style={{ padding:'12px 8px', borderRadius:11, background:'var(--surface-2)', border:'1px solid var(--border-2)', cursor:'pointer', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:7, transition:'all 0.15s' }}>
                  ...
                  {uploading && (
                    <div style={{ padding:10, borderRadius:10, background:'rgba(237,25,102,0.06)', border:'1px solid rgba(237,25,102,0.2)', display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                ...
                {docs.length > 0 && (
                  <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                    ...
                    {docs.map(doc => {
                      ...
                      return (
                        <div key={doc.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:11, background:'var(--surface-2)', border:'1px solid var(--border-2)' }}>
                ...
                <div>
                  ...
                  <textarea value={agent.knowledge} onChange={e=>upd('knowledge',e.target.value)} placeholder="Products: Plan Growth $79/month
Business hours: Mon-Fri 9am-6pm" rows={7} style={{ ...inp, resize:'vertical', lineHeight:1.7, fontFamily:'monospace', fontSize:12 }}/>
                </div>
              </div>
            )}
...
- Free shipping over $50" rows={10} style={{ ...inp, resize:'vertical', lineHeight:1.7 }}/>
              </div>
            )}
...
- Always ask if there is anything else" rows={12} style={{ ...inp, resize:'vertical', lineHeight:1.7 }}/>
            ...
            {tab === 'test' && (
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                ...
                <div style={{ minHeight:260, maxHeight:380, overflowY:'auto', display:'flex', flexDirection:'column', gap:10, WebkitOverflowScrolling:'touch' }}>
                  ...
                  {testing && (
                    <div style={{ display:'flex', gap:7 }}>
                      <div style={{ width:28, height:28, borderRadius:9, background:curAvatar.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:900, color:'#fff' }}>{agent.avatar||'S'}</div>
                      <div style={{ padding:'10px 14px', borderRadius:'13px 13px 13px 4px', background:'var(--surface-2)', border:'1px solid var(--border-2)', display:'flex', gap:4 }}>
                        {[0,1,2].map(i => <span key={i} style={{ width:6, height:6, borderRadius:'50%', background:'var(--text-3)', display:'inline-block' }}/>)}
                      </div>
                    </div>
                  )}
                </div>
