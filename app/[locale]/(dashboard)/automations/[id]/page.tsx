...
export default function AutomationEditPage({ params }: PageProps) {
  ...
  const toggleStatus = async () => {
    ...
    toast.success(newStatus === 'active'
      ? (locale === 'es' ? 'Activada' : 'Activated')
      : (locale === 'es' ? 'Pausada' : 'Paused'))
  }
  ...
  if (!auto) return <div className="p-8 text-center" style={{ color: 'var(--text-3)' }}>Not found</div>
    ...
    <div className="p-8 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="font-display font-bold text-2xl" style={{ color: 'var(--text)' }}>{locale === 'es' ? 'Editar Automatización' : 'Edit Automation'}</h1>
          <p className="text-sm" style={{ color: 'var(--text-3)' }}>{auto.run_count} {locale === 'es' ? 'ejecuciones' : 'runs'}</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={toggleStatus} className="flex items-center gap-2 text-sm px-4 py-2 rounded-xl" style={status === 'active' ? { background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', color: '#22c55e' } : { background: 'var(--surface-2)', border: '1px solid var(--border-2)', color: 'var(--text-2)' }}>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Runs', value: String(auto.run_count) },
          { label: 'Actions', value: String(auto.actions.length) },
        ].map(s => (
      ...
      <div className="card rounded-2xl p-6 space-y-4">
        <h2 className="font-display font-bold text-base" style={{ color: 'var(--text)' }}>{locale === 'es' ? 'Información' : 'Information'}</h2>
        <div className="space-y-2">
          <label className="text-xs font-semibold" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Nombre' : 'Name'}</label>
          <input className="input" value={name} onChange={e => setName(e.target.value)} />
        </div>
        <div className="space-y-2">
          <label className="text-xs font-semibold" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Descripción' : 'Description'}</label>
      ...
      <div className="card rounded-2xl p-6 space-y-4">
        ...
        <div className="flex items-center gap-4 rounded-xl p-4" style={{ background: 'var(--surface-2)', border: '1px solid var(--border-2)' }}>
          ...
          <p className="font-semibold text-sm capitalize" style={{ color: 'var(--text)' }}>{auto.trigger.type.replace(/_/g, ' ')}</p>
        </div>
      </div>

      <div className="card rounded-2xl p-6 space-y-4">
        <h2 className="font-display font-bold text-base" style={{ color: 'var(--text)' }}>{locale === 'es' ? 'Acciones' : 'Actions'} ({auto.actions.length})</h2>
        <div className="space-y-3">
          {[...auto.actions].sort((a, b) => a.order - b.order).map((action, i) => (
            <div key={action.id} className="flex items-center gap-4 rounded-xl p-4" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
              <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: 'var(--pink)' }}>{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: 'var(--text)' }}>{ACTION_LABELS[action.type]?.[loc] ?? action.type}</p>
                <p className="text-xs truncate mt-0.5" style={{ color: 'var(--text-3)' }}>{String(action.config.message ?? action.config.tag ?? action.config.reply ?? '')}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-2xl p-6" style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.2)' }}>
        <h2 className="font-display font-bold text-base mb-2" style={{ color: '#ef4444' }}>{locale === 'es' ? 'Zona de peligro' : 'Danger Zone'}</h2>
        <p className="text-sm mb-4" style={{ color: 'var(--text-3)' }}>{locale === 'es' ? 'Esta acción no se puede deshacer.' : 'This action cannot be undone.'}</p>
        <button onClick={deleteAutomation} className="flex items-center gap-2 text-sm px-4 py-2 rounded-xl" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444' }}>
          <Trash2 size={14} />
          {locale === 'es' ? 'Eliminar' : 'Delete automation'}
        </button>
      </div>
    </div>
  )
