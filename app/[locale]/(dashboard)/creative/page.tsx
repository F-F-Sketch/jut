  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          {label: loc === 'es' ? 'AnÃÂÃÂ¡lisis realizados' : 'Analyses run', value: stats.analyses, icon: Brain, color: 'var(--pink)'},
          {label: loc === 'es' ? 'Score promedio' : 'Average score', value: stats.avg_score ? stats.avg_score + '/100' : 'ÃÂ¢ÃÂÃÂ', icon: TrendingUp, color: '#22c55e'},
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          {href:'/'+locale+'/creative/analyzer',icon:Eye,label:loc==='es'?'Analizador':'Analyzer',desc:loc==='es'?'Sube un creativo y obtÃÂÃÂ©n anÃÂÃÂ¡lisis con scores y heatmap':'Upload a creative and get scores, heatmap, recommendations',color:'var(--pink)',badge:'AI'},
          {href:'/'+locale+'/creative/analyzer',icon:Sparkles,label:loc==='es'?'Mejorador':'Enhancer',desc:loc==='es'?'JUT genera una versiÃÂÃÂ³n mejorada de tu creativo':'JUT generates an optimized version of your creative',color:'#4a90d9',badge:loc==='es'?'IA Generativa':'Generative AI'},
          ...
          {href:'/'+locale+'/creative/reports',icon:BarChart3,label:loc==='es'?'Reportes':'Reports',desc:loc==='es'?'Exporta anÃÂÃÂ¡lisis y reportes de performance':'Export analyses and performance reports',color:'#22c55e',badge:'PDF'},
      ...
      {recentAnalyses.length === 0 && (
        <div className="rounded-2xl p-12 flex flex-col items-center justify-center text-center" style={{background:'linear-gradient(135deg,rgba(237,25,102,0.04),rgba(33,82,164,0.04))',border:'1px dashed var(--border-2)'}}>
          ...
          <h3 className="font-display font-bold text-xl mb-2" style={{color:'var(--text)'}}>
            {loc === 'es' ? 'Tu inteligencia creativa empieza aquÃÂÃÂ­' : 'Your creative intelligence starts here'}
          </h3>
          <p className="text-sm max-w-md mb-6" style={{color:'var(--text-3)'}}>
            {loc === 'es' ? 'Sube cualquier creativo de marketing ÃÂ¢ÃÂÃÂ JUT lo analizarÃÂÃÂ¡ con IA.' : 'Upload any marketing creative ÃÂ¢ÃÂÃÂ JUT will analyze it with AI.'}
          </p>
