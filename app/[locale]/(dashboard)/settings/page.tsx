    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      {tab === 'profile' && (
        <div className="rounded-2xl p-6 space-y-5" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          <h2 className="font-display font-bold text-base mb-4" style={{ color: 'var(--text)' }}>{locale === 'es' ? 'InformaciÃ³n Personal' : 'Personal Information'}</h2>
      ...
      {tab === 'security' && (
        <div className="rounded-2xl p-6 space-y-5" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          ...
          <div className="space-y-4 max-w-md">
            {('new' as const[]).map(f => (
              <div key={f}><label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-2)' }}>{label}</label><div className="relative"><input type={showPass ? 'text' : 'password'} value={passwords[f]} onChange={e => setPasswords(p => ({ ...p, [f]: e.target.value }))} className="input pr-10" placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" /></div></div>
            ))}
            <div><label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Nueva contraseÃ±a' : 'New password'}</label><div className="relative"><input type={showPass ? 'text' : 'password'} value={passwords.new} onChange={e => setPasswords(p => ({ ...p, new: e.target.value }))} className="input pr-10" placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" /><button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-3)' }}>{showPass ? <EyeOff size={15} /> : <Eye size={15} />}</button></div></div>
            <div><label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Confirmar' : 'Confirm'}</label><input type="password" value={passwords.confirm} onChange={e => setPasswords(p => ({ ...p, confirm: e.target.value }))} className="input" placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" /></div>
      ...
      {tab === 'integrations' && (
        <div className="space-y-4">
          {[{ name: 'Instagram', icon: 'ð¸', desc: locale === 'es' ? 'Conecta tu cuenta para capturar comentarios' : 'Connect to capture comments and send automated DMs' },{ name: 'WhatsApp Business', icon: 'ð¬', desc: locale === 'es' ? 'EnvÃ­a mensajes automÃ¡ticos por WhatsApp' : 'Send automated WhatsApp messages' }].map(({ name, icon, desc }) => (
      ...
      {tab === 'webhooks' && (
        <div className="space-y-5">
          <div className="rounded-2xl p-4" style={{ background: 'rgba(74,144,217,0.08)', border: '1px solid rgba(74,144,217,0.2)' }}><p className="font-semibold text-sm" style={{ color: '#4a90d9' }}>ð {locale === 'es' ? 'Conecta JUT con n8n, Zapier y mÃ¡s' : 'Connect JUT with n8n, Zapier and more'}</p><p className="text-xs mt-1" style={{ color: 'var(--text-3)' }}>{locale === 'es' ? 'JUT enviarÃ¡ datos a tu URL cuando ocurran eventos.' : 'JUT sends data to your URL when events happen.'}</p></div>
          <div className="rounded-2xl p-6 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Nombre' : 'Name'}</label><input value={newWh.name} onChange={e => setNewWh(p => ({ ...p, name: e.target.value }))} placeholder="n8n Production" className="input" /></div>
            ...
            <div><label className="block text-xs font-semibold mb-2" style={{ color: 'var(--text-2)' }}>{locale === 'es' ? 'Eventos' : 'Events'}</label><div className="flex flex-wrap gap-2">{EVEMTS,map(ev?>(<button key={ev} onClick={() => setNewWh(p => ({ ...p, events: p.events.includes(ev) ? p.events.filter(x => x !== ev) : [...p.events, ev] }))} className="text-xs px-3 py-1.5 rounded-lg font-medium transition-all" style={{ background: newWh.events.includes(ev) ? 'rgba(237,25,102,0.1)' : 'var(--surface-2)', color: newWh.events.includes(ev) ? 'var(--pink)' : 'var(--text-3)', border: `1px solid ${newWh.events.includes(ev) ? 'rgba(237,25,102,0.3)' : 'var(--border-2)'}` }}>{ev.replace(/_/g,' ')}</button>)))}</div></div>
