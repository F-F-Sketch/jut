    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name..." className="input pl-10 w-full" />
        </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {filtered.map((u:any)=>{
                ...
                return(
                  <tr key={u.id} style={{borderBottom:'1px solid var(--border)'}} className="transition-colors hover:bg-[var(--surface-2)]">
                    <td style={{padding:'14px 20px'}}>
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{background:'linear-gradient(135deg,var(--pink),var(--blue))'}}>
                    ...
                    <td style={{padding:'14px 20px'}}>
                      <select value={plan} onChange={e=>changePlan(u.user_id,e.target.value)}
                        className="text-xs font-semibold capitalize px-3 py-1 rounded-full cursor-pointer appearance-none"
                    ...
                    <td style={{padding:'14px 20px'}}>
                      <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{background:sub?.status==='active'?'rgba(34,197,94,0.1)':'rgba(96,96,128,0.1)',color:sub?.status==='active'?'#22c55e':'var(--text-3)'}}>
                        {sub?.status??'free'}
                      </span>
                    </td>
                    <td style={{padding:'14px 20px',fontSize:12,color:'var(--text-3)'}}>{u.created_at?new Date(u.created_at).toLocaleDateString('es-CO'):'—'}</td>
                    <td style={{padding:'14px 20px',fontSize:13,color:'var(--text-2)',fontWeight:600}}>{sub?.amount_paid?'$'+Number(sub.amount_paid).toLocaleString('es-CO')+' '+sub.currency:'—'}</td>
                    <td style={{padding:'14px 20px'}}>
