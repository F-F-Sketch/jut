...
export default async function AdminPage({ params }: { params: { locale: string } }) {
  ...
  const planCounts = subs.reduce((acc: Record<string, number>, s) => { acc[s.plan] = (acc[s.plan] ?? 0) + 1; return acc }, {})
  const recentUsers = profiles.slice(0, 8)
  const stats = [
    { label: 'Total Members', value: totalUsers, icon: 'ð¥', color: '#ED1966' },
    { label: 'Paying Members', value: paidUsers, icon: 'ð³', color: '#22c55e' },
    { label: 'MRR (COP)', value: `$${mrrCOP.toLocaleString('es-CO')}`, icon: 'ð°', color: '#f59e0b' },
    { label: 'Free Plan', value: planCounts['free'] ?? 0, icon: 'ð', color: '#4a90d9' },
  ]
  return (
