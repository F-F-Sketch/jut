...
async function getDashboardData(userId: string) {
  try {
    ...
    return {
      ...
      recentConvos: convoRes.data ?? [],
      totalLeads: leadsRes.data?.length ?? 0,
      revenue,
      automationsFired,
      dbReady: !leadsRes.error,
    }
  } catch {
...
export default async function DashboardPage({ params }: PageProps) {
  ...
  let profile = null
  try { if (user) profile = await getUserProfile(user.id) } catch { /* ignore */ }

  const { recentLeads, recentConvos, totalLeads, revenue, automationsFired, dbReady } =
    user ? await getDashboardData(user.id) : { recentLeads: [], recentConvos: [], totalLeads: 0, revenue: 0, automationsFired: 0, dbReady: false }

  const userName = profile?.full_name?.split(' ')[0] ?? user?.email?.split('@')[0] ?? 'there'
  const hour = new Date().getHours()
  const greeting = hour < 12
    ? (loc === 'es' ? 'Buenos días' : 'Good morning')
    : hour < 18
    ? (loc === 'es' ? 'Buenas tardes' : 'Good afternoon')
    : (loc === 'es' ? 'Buenas noches' : 'Good evening')
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-sm" style={{ color: 'var(--text-3)' }}>
          {loc === 'es' ? 'Aquí tienes el resumen de tu negocio.' : "Here's your business overview."}
        </p>
      </div>
      ...
      {!dbReady && (
        <div className="rounded-2xl p-5 flex items-start gap-4" style={{ background: 'rgba(237,25,102,0.08)', border: '1px solid rgba(237,25,102,0.25)' }}>
          <AlertCircle size={20} style={{ color: 'var(--pink)', flexShrink: 0, marginTop: 2 }} />
          <div>
            <p className="font-semibold text-sm mb-1" style={{ color: 'var(--text)' }}>
              {loc === 'es' ? 'Base de datos no configurada' : 'Database not set up yet'}
            </p>
            <p className="text-sm" style={{ color: 'var(--text-3)' }}>
              {loc === 'es'
                ? 'Ve a Supabase → SQL Editor y ejecuta el archivo schema.sql para activar todas las funciones.'
                : 'Go to Supabase → SQL Editor and run the schema.sql file to enable all features.'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { label: loc === 'es' ? 'Total Leads' : 'Total Leads', value: totalLeads, icon: '👥', color: 'var(--pink)' },
          { label: loc === 'es' ? 'Conversaciones' : 'Conversations', value: recentConvos.length, icon: '💬', color: 'var(--blue-light)' },
          { label: loc === 'es' ? 'Automatizaciones' : 'Automations Fired', value: automationsFired, icon: '⚡', color: '#22c55e' },
          { label: loc === 'es' ? 'Ingresos' : 'Revenue', value: `$${revenue.toLocaleString()}`, icon: '💰', color: '#f59e0b' },
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
            <h2 className="font-display font-bold text-base" style={{ color: 'var(--text)' }}>
              {loc === 'es' ? 'Leads Recientes' : 'Recent Leads'}
            </h2>
            <Link href={`/${locale}/leads`} className="flex items-center gap-1 text-xs font-medium" style={{ color: 'var(--pink)' }}>
              {loc === 'es' ? 'Ver todos' : 'View all'} <ArrowRight size={12} />
            </Link>
          </div>
          {recentLeads.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <Link href={`/${locale}/leads/new`} className="text-xs font-semibold px-4 py-2 rounded-lg" style={{ background: 'var(--pink)', color: '#fff' }}>
                {loc === 'es' ? 'Agregar lead' : 'Add lead'}
              </Link>
            </div>
          ) : (
            <div>
              {recentLeads.map((lead: any) => (
                <Link key={lead.id} href={`/${locale}/leads/${lead.id}`} className="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-[var(--surface-2)]">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium capitalize"
                    style={{ background: 'rgba(237,25,102,0.1)', color: 'var(--pink)' }}>{lead.status}</span>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
            <h2 className="font-display font-bold text-base" style={{ color: 'var(--text)' }}>
              {loc === 'es' ? 'Conversaciones Recientes' : 'Recent Conversations'}
            </h2>
            <Link href={`/${locale}/conversations`} className="flex items-center gap-1 text-xs font-medium" style={{ color: 'var(--pink)' }}>
              {loc === 'es' ? 'Ver todas' : 'View all'} <ArrowRight size={12} />
            </Link>
          </div>
          {recentConvos.length === 0 ? (
            ...
            <div>
              {recentConvos.map((convo: any) => (
                <Link key={convo.id} href={`/${locale}/conversations/${convo.id}`} className="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-[var(--surface-2)]">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-lg flex-shrink-0" style={{ background: 'var(--surface-3)' }}>💬</div>
                  <div className="flex-1 min-w-0">
                  ...
                  {convo.unread_count > 0 && (
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: 'var(--pink)' }}>{convo.unread_count}</span>
                  )}
                </Link>
              ))}
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-bold tracking-wider uppercase" style={{ color: 'var(--pink)' }}>
                {loc === 'es' ? 'Sistema activo' : 'System Active'}
              </span>
            </div>
            <h3 className="font-display font-bold text-lg mb-1" style={{ color: 'var(--text)' }}>
              {loc === 'es' ? 'JUT está listo para automatizar' : 'JUT is ready to automate'}
            </h3>
            <p className="text-sm" style={{ color: 'var(--text-3)' }}>
              {loc === 'es' ? 'Conecta Instagram y crea tu primera automatización.' : 'Connect Instagram and create your first automation.'}
            </p>
          </div>
          <Link href={`/${locale}/automations`} className="flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold whitespace-nowrap"
