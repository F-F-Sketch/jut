  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-3)' }}>
            {loc === 'es' ? 'Analiza, optimiza y experimenta con tus creativos de marketing usando IA.' : 'Analyze, optimize, and experiment with your marketing creatives using AI.'}
          </p>
        </div>
        <Link href={`/${locale}/creative/analyzer`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          {
            href: `/${locale}/creative/analyzer`,
            icon: Eye,
            label: loc === 'es' ? 'Analizador' : 'Analyzer',
            desc: loc === 'es' ? 'Sube un creativo y obtén un análisis completo con scores, heatmap y recomendaciones' : 'Upload a creative and get full analysis with scores, heatmap, and recommendations',
            color: 'var(--pink)',
            badge: 'AI',
          },
          {
            href: `/${locale}/creative/analyzer`,
            icon: Sparkles,
            label: loc === 'es' ? 'Mejorador' : 'Enhancer',
            desc: loc === 'es' ? 'JUT genera una versión mejorada de tu creativo basada en los análisis' : 'JUT generates an optimized version of your creative based on analysis',
            color: '#4a90d9',
            badge: loc === 'es' ? 'IA Generativa' : 'Generative AI',
          },
          {
            href: `/${locale}/creative/experiments`,
            icon: FlaskConical,
            label: loc === 'es' ? 'Experimentos' : 'Experiments',
            desc: loc === 'es' ? 'Crea tests A/B y multivariados para tus creativos' : 'Create A/B and multivariate tests for your creatives',
            color: '#f59e0b',
            badge: 'A/B',
          },
          {
            href: `/${locale}/creative/reports`,
            icon: BarChart3,
            label: loc === 'es' ? 'Reportes' : 'Reports',
            desc: loc === 'es' ? 'Exporta análisis, comparativas y reportes de performance' : 'Export analyses, comparisons, and performance reports',
            color: '#22c55e',
            badge: 'PDF',
          },
        ].map(({ href, icon: Icon, label, desc, color, badge }) => (
          <Link key={href + label} href={href}
            className="rounded-2xl p-6 flex flex-col gap-4 transition-all hover:opacity-90 group"
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-bold text-lg" style={{ color: 'var(--text)' }}>
              {loc === 'es' ? 'Análisis recientes' : 'Recent analyses'}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {recentAnalyses.map((a: any) => {
              ...
              return (
                <div key={a.id} className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs" style={{ color: 'var(--text-3)' }}>{new Date(a.created_at).toLocaleDateString()}</span>
                  ...
                  <p className="text-sm" style={{ color: 'var(--text-2)' }}>{(a.insights as any)?.summary?.slice(0, 80) ?? 'Analysis complete'}...</p>
                </div>
              )
            })}
          </div>
      ...
      {recentAnalyses.length === 0 && (
        <div className="rounded-2xl p-12 flex flex-col items-center justify-center text-center"
          style={{ background: 'linear-gradient(135deg, rgba(237,25,102,0.04), rgba(33,82,164,0.04))', border: '1px dashed var(--border-2)' }}>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6"
            style={{ background: 'rgba(237,25,102,0.08)', border: '1px solid rgba(237,25,102,0.2)' }}>
          ...
          <p className="text-sm max-w-md mb-6" style={{ color: 'var(--text-3)' }}>
            {loc === 'es'
              ? 'Sube cualquier creativo de marketing — JUT lo analizará con IA y te dirá exactamente qué mejorar para aumentar conversiones.'
              : 'Upload any marketing creative — JUT will analyze it with AI and tell you exactly what to improve to increase conversions.'}
          </p>
          <Link href={`/${locale}/creative/analyzer`}
