  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-2">
        <Link href={`/${locale}/admin/users`} className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-3)' }}>
      ...
      <div className="rounded-2xl p-6 flex items-start gap-5" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <span className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-3)' }}>
              Plan:
              <span className="font-bold capitalize" style={{ color: 'var(--pink)' }}>{profile.plan ?? 'free'}</span>
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { label: 'Automations', value: automations.length, icon: Zap },
          { label: 'Revenue', value: sub?.amount_paid ? `$${Number(sub.amount_paid).toLocaleString()} ${sub.currency}` : '$0', icon: CreditCard },
      ...
      {sub && (
        <div className="rounded-2xl p-6 space-y-3" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          <h2 className="font-display font-bold text-base" style={{ color: 'var(--text)' }}>Subscription</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            {[
              ['Plan', sub.plan],
              ['Status', sub.status],
              ['Provider', sub.payment_provider ?? '—'],
              ['Amount', sub.amount_paid ? `$${sub.amount_paid} ${sub.currency}` : '—'],
              ['Period Start', sub.current_period_start ? new Date(sub.current_period_start).toLocaleDateString() : '—'],
              ['Period End', sub.current_period_end ? new Date(sub.current_period_end).toLocaleDateString() : '—'],
            ].map(([label, value]) => (
              <div key={label}>
      ...
      {automations.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
          ...
          {automations.map((a: any) => (
            <div key={a.id} className="flex items-center gap-4 px-6 py-3.5" style={{ borderBottom: '1px solid var(--border)' }}>
              <div className="flex-1">
