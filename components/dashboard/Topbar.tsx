...
interface Notification {
  ...
  time: string
  read: boolean
  type: 'automation' | 'lead' | 'sale' | 'system'
}
...
export function Topbar({ locale, userName, title, subtitle }: TopbarProps) {
  const [userMenuOpen, setUserMenuOpen] = useState(false)
  const [cmdOpen, setCmdOpen] = useState(false)
  const [notifOpen, setNotifOpen] = useState(false)
  const [notifications, setNotifications] = useState<Notification[]>([
    { id: '1', text: locale === 'es' ? 'Automatización ejecutada exitosamente' : 'Automation run completed', time: '2m ago', read: false, type: 'automation' },
    { id: '2', text: locale === 'es' ? 'Lead calificado: Maria Rodriguez' : 'Lead qualified: Maria Rodriguez', time: '15m ago', read: false, type: 'lead' },
    { id: '3', text: locale === 'es' ? 'Nueva venta registrada: $297' : 'New sale recorded: $297', time: '1h ago', read: true, type: 'sale' },
  ])
  ...
  return (
    <>
      <header
        className="fixed top-0 right-0 left-64 h-[68px] z-30 flex items-center px-6 gap-4"
        style={{ background: 'rgba(13,13,20,0.92)', backdropFilter: 'blur(24px)', borderBottom: '1px solid var(--border)' }}
      >
        {/* Title */}
        <div className="flex-1 min-w-0">
          {title && <h1 className="font-display font-bold text-base truncate" style={{ color: 'var(--text)' }}>{title}</h1>}
          {subtitle && <p className="text-xs truncate" style={{ color: 'var(--text-3)' }}>{subtitle}</p>}
        </div>

        {/* Search bar */}
        <button
          onClick={() => setCmdOpen(true)}
          className="hidden md:flex items-center gap-2 rounded-xl px-4 py-2 text-sm transition-all hover:border-opacity-30"
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            ...
            {unread > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: 'var(--pink)', fontSize: 9 }}>
                {unread}
              </span>
            )}
          </button>
            ...
            >
              ...
              <div className="max-h-72 overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="py-10 text-center">
                    <p className="text-sm" style={{ color: 'var(--text-3)' }}>{locale === 'es' ? 'Sin notificaciones' : 'No notifications'}</p>
                  </div>
                ) : notifications.map(n => (
                  <div
                    key={n.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs leading-relaxed" style={{ color: n.read ? 'var(--text-3)' : 'var(--text-2)' }}>{n.text}</p>
                      <p className="text-xs mt-1" style={{ color: 'var(--text-3)' }}>{n.time}</p>
                    </div>
                    {!n.read && <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1.5" style={{ background: 'var(--pink)' }} />}
                    <button onClick={(e) => dismissNotif(n.id, e)} className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: 'var(--text-3)' }}>
                      <X size={12} />
                    </button>
                  </div>
                ))}
              ...
              <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                <p className="text-xs text-center" style={{ color: 'var(--text-3)' }}>
                  {locale === 'es' ? 'Las notificaciones en tiempo real llegarán aquí' : 'Real-time notifications will appear here'}
                </p>
              </div>
            </div>
          )}
        ...
        <div ref={userRef} className="relative">
          ...
          >
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: 'linear-gradient(135deg, var(--pink), var(--blue))' }}>
          ...
          {userMenuOpen && (
            ...
            >
              ...
              <Link href={`/${locale}/settings`} onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-[var(--surface-2)]" style={{ color: 'var(--text-2)' }}>
                <User size={14} />{locale === 'es' ? 'Perfil' : 'Profile'}
              </Link>
              <Link href={`/${locale}/settings#integrations`} onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-[var(--surface-2)]" style={{ color: 'var(--text-2)' }}>
                <Settings size={14} />{locale === 'es' ? 'Ajustes' : 'Settings'}
              </Link>
              <div className="my-1" style={{ borderTop: '1px solid var(--border)' }} />
              <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-[var(--surface-2)]" style={{ color: '#ef4444' }}>
